"""
Hybrid Nav2 + Lane Following Launch File

Brings up:
  1. QCar2 hardware (virtual): csi camera, lidar, hardware driver, TF
  2. Cartographer SLAM (map building)
  3. Nav2 stack (planner, controller, bt_navigator) — for path planning & RViz goals
  4. Lane following pipeline: color_segmentation + yellow_line_position
  5. (Optional) nav2_lane_bridge for hybrid control

The key difference from the original qcar2_slam_and_nav_bringup_virtual_launch.py:
  ...
  - Later: bridge node fuses Nav2 velocity with lane-following steering
...
def generate_launch_description():
    ...
    # ═══════════════════════════════════════════════════════
    # 1) QCar2 HARDWARE (virtual) — from qcar2_nodes
    # ═══════════════════════════════════════════════════════
    qcar2_virtual_launch = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(qcar2_dir, 'launch', 'qcar2_virtual_launch.py')
        )
    )

    # TF: qcar2 → lidar
    qcar2_to_lidar_tf = Node(
        package='qcar2_nodes',
    ...
    cartographer_config_dir = PathJoinSubstitution(
        [FindPackageShare('qcar2_nodes'), 'config']
    )
    ...
    # ═══════════════════════════════════════════════════════
    # 3) NAV2 (planner + controller + bt_navigator) — NO converter
    # ═══════════════════════════════════════════════════════
    ...
    # ═══════════════════════════════════════════════════════
    # 4) LANE FOLLOWING PIPELINE (segmentation + position)
    # ═══════════════════════════════════════════════════════
    ...
    # ═══════════════════════════════════════════════════════
    # 5) HYBRID BRIDGE (optional — enable with enable_bridge:=true)
    # ═══════════════════════════════════════════════════════
    ...
    # 3) Nav2 (NO converter — planning only)
